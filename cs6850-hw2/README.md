...
So with a mapping of x/A would cause Func(A) to match ¬Func(x), but this won't happen because the mapping only occurs after the terms are first scanned for an exact match.  But I can't find an example (yet) of this occurring in any data set that starts out valid.
